...
def quantile_loss(diff, quantile=0.5):
    ...
    return weight * diff.abs()

def exponential_loss(diff, temperature=1.0):
    return temperature * (torch.exp(diff) - diff)
...
class IQL(object):
    ...
    def update_v(self, states, actions, logger=None):
        with torch.no_grad():
            ...
            q1 = self.critic_target(states, actions)
            q = q1

        v = self.value(states)
        if (self.loss_type == "expectile_loss"):
          value_loss = expectile_loss(q - v, self.expectile).mean()
        elif (self.loss_type == "quantile_loss"):
          value_loss = quantile_loss(q - v, self.expectile).mean()
        else:
          value_loss = exponential_loss(q - v, self.expectile).mean()
    ...
    def update_q(self, states, actions, rewards, next_states, not_dones, logger=None):
        ...
        critic_loss = ((q1 - target_q)**2).mean()
