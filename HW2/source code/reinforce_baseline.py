...
class Policy(nn.Module):
    ...
    def calculate_loss(self, gamma=0.999):
        ...
        # Calculate baseline
        baselines = self.get_v_values(gamma)

        # Calculate advantages and returns
        returns = []
        for reward, baseline in zip(reversed(self.rewards), reversed(baselines)):
            R = reward + gamma * R
        ...
        # Calculate policy loss with V-value baseline
    ...
    def get_v_values(self, gamma):
        state_values = []
        R = 0

        for action in reversed(self.saved_actions):
            # Assuming action.value contains the value of the next state
            R = action.value + gamma * R  # V^pi_theta(s) = r + gamma * V^pi_theta(s')
            state_values.insert(0, R)
    ...
    for i_episode in count(1):
        ...
        if ewma_reward > env.spec.reward_threshold or i_episode > 2000:
            if not os.path.isdir(save_path):
                os.mkdir(save_path)
            torch.save(model.state_dict(), save_path + '/LunarLander_reinforce_baseline={}.pth'.format(lr))
...
def test(name, n_episodes=10):
    ...
    model.load_state_dict(torch.load('/content/drive/My Drive/資訊工程學習資料/強化學習原理/課程作業 (謝秉均)/HW2/reinforce_baseline/{}'.format(name)))
