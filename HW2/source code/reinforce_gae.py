...
class Policy(nn.Module):
    ...
    def calculate_loss(self, gamma=0.999, lambda_ = 0.99):
        ...
        # get probility and value
        log_probs = [action.log_prob for action in saved_actions]
        values = [action.value for action  in saved_actions]

        # calculate (P4) advantages = E[advantages * log_probs(a|s)] / (1 - gamma)
        advantages = GAE(gamma, lambda_, None)(self.rewards, values, False)
        advantages = advantages.detach()

        action_log_probs = torch.stack(log_probs, dim=0)
        values = torch.stack(values, dim=0)[:,0]

        policy_losses = -(advantages * action_log_probs).sum() / (1 - gamma)
        # value_losses = F.mse_loss(values, returns).sum()
        value_losses = F.smooth_l1_loss(values, returns).sum()
...
class GAE:
    ...
    def __call__(self, rewards, values, done):
        ...
        # refer to slide Lec11 P.6, use defition of GAE
        advantages = []
        advantage = 0
        next_value = 0
        ...
        for r, v in zip(reversed(rewards), reversed(values)):
            ...
            if self.num_steps is not None and stepCount > self.num_steps:
                break
...
def train(lr=0.01, lr_decay=0.9, gamma=0.99, lambda_=0.99, i_episode_step=1):
    """
    ...
    for i_episode in count(1):
        ...
        if ewma_reward > env.spec.reward_threshold:
            if not os.path.isdir(save_path):
                os.mkdir(save_path)
            torch.save(model.state_dict(), save_path + '/LunarLander_reinforce_gae={}.pth'.format(gamma))
...
def test(name, n_episodes=10):
    ...
    model.load_state_dict(torch.load('/content/drive/My Drive/資訊工程學習資料/強化學習原理/課程作業 (謝秉均)/HW2/reinforce_gae/{}'.format(name)))
...
if __name__ == '__main__':
    ...
    train(lr, lr_decay, gamma, lambda_, i_episode_step)
    test(f'LunarLander_reinforce_gae={gamma}.pth')